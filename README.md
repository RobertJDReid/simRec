...
- Post-hoc LOH classification: GC-NCO, GC-CO, and CO-terminal events
...
- Tab-separated event output for scripting and batch runs
...
```python
from simRec import load_genome, run_simulation, plot_cell_and_loh, compute_loh, classify_events
...
for event in classify_events(final_cell):
    print(event)
...
|---|---|---|---|
| `type` | `str` | `GC-NCO`, `GC-CO`, `CO-terminal` | Event classification |
...
- **GC-NCO** — internal LOH block where homolog A has the same haplotype identity on both sides of the block (no phase switch).
- **GC-CO** — internal LOH block where homolog A switches haplotype identity across the block, indicating an associated crossover.
- **CO-terminal** — LOH block extending to a telomere. Multiple abutting terminal blocks on the same arm are each reported separately, reflecting the possibility that earlier events are masked by later ones.
- **Adjacent-to-terminal** — internal GC events whose immediate LOH neighbor is a non-het block are flagged, as their interpretation is ambiguous: they may represent genuine prior GC events flanked by a later terminal CO, or artefacts of multiple overlapping terminal events.
...
42 tests covering interval helpers, GC placement and CEN exclusion, gene conversion, crossover directionality, replication independence, segregation correctness, LOH computation, event classification, and end-to-end simulation integrity.
